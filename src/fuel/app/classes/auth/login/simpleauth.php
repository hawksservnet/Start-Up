...
class Auth_Login_Simpleauth extends \Auth_Login_Driver
{
	...
	protected function perform_check()
	{
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			{
			    //check user info success
                if (strpos(\Request::active()->controller, 'Controller_Admin') === false && !empty(\Session::get('manage_flg'))) {
                    $this->user = false;
                    ...
                    \Session::delete('Account_login_hash');
                    \Session::delete('LOGIN');
                    \Session::delete('manage_flg');
	...
	public function login($username_or_email = '', $password = '')
	{
        ...
        \Session::set('MYPAGE', array('ID'=>$this->user['id'], 'NAME'=>$this->user['name_last'] . ' ' . $this->user['name_first']));
