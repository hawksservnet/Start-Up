...
page = {
	...
	init : function (){
		...
		for (var i = 0; i < $fT.length; i++) {
			$fT.eq(i).attr('tabindex',(i+1));
		}
		var targetElm = "input[type=text],input[type=zip],input[type=password],input[type=submit],select,input[type=button],input[type=checkbox],textarea,a,button,label";
			$targetElm = $( targetElm );
			$(function () {
			var elements = "input[type=text],input[type=zip],input[type=password],select,input[type=checkbox],label";
		...
		if(page.$s[0]){
			...
			if($("#access-map")[0]){
				setTimeout(function(){
				var mapLatLng = new google.maps.LatLng(35.679235,139.762304);
				...
				[
				  ...
				  var mapMap = new google.maps.Map(document.getElementById('access-map'), myOptions);

				  //var URL = location.href;
				  var icon1 = {
				   url : 'assets/img/common/icon_map.svg',
				   scaledSize : new google.maps.Size(83,106)
				   // ↑ここで画像のサイズを指定
				  }
				  ...
				  mapMap.mapTypes.set('access-map', mapStyle);
				  mapMap.setMapTypeId('access-map');
		...
		// console.log("ie",Useragnt.ie);

		page.gnavi.init();

		// console.log("Useragnt.android",Useragnt);
		// console.log("Useragnt.android",Useragnt.android.str);
		// alert(Useragnt.android.version.str);
		...
		var ks = {
			...
			type    : "auto",
			PAUSE   : 4, //[ 待ち時間 ]
			SPEED   : 0.48, //[ 切り替えアニメーション時間 ]
			timer : []
		};

		var kvSlider = {
			$w : $("#kv-bg-slider"),
			$t : $("#kv-bg-slider .slide"),
			$p : $("#kv-slider-pager"),
			init : function(){
				ks.len = kvSlider.$t.length;
				kvSlider.motion();

			},
			motion :function(){
				var $c = kvSlider.$t.eq(ks.current);
				$c.addClass("current");
				ks.timer = setTimeout(function(){
					kvSlider.update();
					kvSlider.motion();
					TweenMax.to(kvSlider.$t.eq(ks.current).children(),ks.SPEED,{
						"width":"100%",
						ease : Power3.easeIn,
					});

					TweenMax.to($c.children(),ks.SPEED,{
						"width":"0%",
						ease : Power3.easeIn,
						delay:0.3,
						onStart:function(){
						...
						onComplete:function(){
							if(ks.current!=ks.prev) kvSlider.$t.eq(ks.prev).removeClass("current");
						}
					});
				},4e3);
			...
			update : function(){
				ks.prev = ks.current;
				if(ks.len-1 > ks.current){
					ks.current++;
				}else{
					ks.current = 0;
				}
				var $c = kvSlider.$p.children().eq(ks.current);
				kvSlider.$p.children().removeClass("current");
				$c.addClass("current");
			}
		}
		...
		/* mypage modal js */
		if($("body").attr("id") == "mypage") $('.openmodal').colorbox({inline: true});


		if(Useragnt.edge || Useragnt.ie || $("body").attr("id") == "application" ||  $("body").attr("id") == "user-registration-body" || $("body").attr("id") == "login" || $("body").attr("id") == "mypage") return
		// if()
		page.bar = new Scrollbar.init(document.getElementById("scroll-body"), {
			speed: 1,
			damping: 0.1,
		...
		setTimeout(function(){
			...
			// }
			var scrollbarFlg = false;
			$(".scrollbar-thumb-y").on("click mousedown",function(){
				if(scrollbarFlg) return;
				scrollbarFlg = true;
				page.scroll.show($("#body").innerHeight()+$(window).height());
			});
	...
	gnavi : {
		...
		$c : $("#gnavi"),
		// $jc : page.gnavi.$c.find(".js__container"),
		status : {
			t   : [],
			p   : [],
			i   : 0,
		...
		init : function(){
			...
			TweenMax.set(page.gnavi.$hc.find(".js__child"),{ x : -page.gnavi.$c.width() });
			TweenMax.set(page.gnavi.$hc.find(".bg"),{ x : page.gnavi.$c.width() });
			TweenMax.set(page.gnavi.$hc.find(".js__dd"),{ height : 0 });


			// for(var i=0; page.gnavi.$jc.length>i; i++){
			// }
			...
			page.gnavi.$h.find("#gnavi-btn").on("click",function(){
				if($(this).hasClass("open")){
					...
					$("#gnavi-head").removeClass("open");
					page.gnavi.close();
				}else{
					$(this).addClass("open").find(".text").text("CLOSE");
					$("#gnavi-head").addClass("open");
					page.gnavi.open();
				}
			});
			...
			page.gnavi.$hc.find(".js__dd-tr").on("click",function(){
				if($(this).hasClass("open")){
					...
					TweenMax.to( page.gnavi.$hc.find(".js__dd"),0.28,{ height : 0 } );
				}else{
					$(this).addClass("open");
					TweenMax.to( page.gnavi.$hc.find(".js__dd"),0.28,{ height : page.gnavi.$hc.find(".js__dd").children().innerHeight() } );
				}
			});

			$("#head-navi-scroll").mCustomScrollbar({
			advanced:{
		...
		open : function(){
			//#head-navi-container-inner

			TweenMax.to($("#head-navi-container-inner"),0.38,
				{
					x : 0,
				});
			TweenMax.to(page.gnavi.$hc.find(".bg"),0.38,
				{
					x : 0,
					onComplete:function(){
						TweenMax.staggerTo(page.gnavi.$hc.find(".js__child"),0.38,
							{
								x : 0,
							},0.1);
					}
		...
		close : function(){
			var inc =  page.gnavi.$c.width()+100;
			TweenMax.to($("#head-navi-container-inner"),0.38,
				{
					x : inc,
				});
			...
			TweenMax.staggerTo(page.gnavi.$hc.find(".js__child"),0.28,
				{
					x : -inc,
				},0.03);
		},
	...
	resize : function(){
		...
		if(page.w > 1280){
			TweenMax.to(page.$fb.not(".show"),.28,{
				right : 25,
				ease:Power2.easeOut,
				onComplete:function(){
			...
			TweenMax.to(page.$fb.filter(".show"),.28,{
				right : -200,
				ease:Power2.easeOut,
				onComplete:function(){
		...
		$("#head-navi-scroll").css({"height": page.h });

		if(page.w > device.BREAKPOINT){
			// $("#head-navi-container").removeAttr("style");
			$("#head-navi-container-inner").css({"height": $("#document-inner").height() });
			// console.log("h",page.h);
		...
		// /* [ 要素よりもwindowが高いとき ] */
		// if($("#document-inner").height() > page.h){
		// 	// console.log("でっか");
		// 	$("#body").css({"height":""});
		// }else{
		// 	// console.log("ちっさ");
		// 	$("#body").filter("resize").css({"height":page.h - $("#footer-container").height()});
		// }
	...
	scroll : {
		...
		motion : function(inc){ //[ page.scroll.motion ]
			...
			page.scroll.inc = (inc!=0)?Number( page.scroll.getTrans( $("#scroll-body").children(".scroll-content") ) ) : 0;
			...
			// var _fbT = (!$("#body").hasClass("resize"))? -$("#document-inner").height() + page.h/2 - page.scroll.inc :
			// 			-[page.h - page.$fb.height()]/2;
			var _fbT = (!$("#body").hasClass("resize"))?
			-$("#scroll-body").children(".scroll-content").height()-page.scroll.inc+[page.h + page.$fb.height() + $("#header").height()]/2  :
						-[page.h - page.$fb.height()]/2;

			// console.log("ee",-$("#scroll-body").children(".scroll-content").height()-page.scroll.inc + Number(page.h)/2,
			// 			-$("#scroll-body").children(".scroll-content").height()-page.scroll.inc+[page.h+page.$fb.height()]/2);


			// if($("#scroll-body").children(".scroll-content").height()>=page.h){
			// // console.log("test",$("#scroll-body").children(".scroll-content").height(),page.h,_fbT);
			// 	page.$fb.css({
		...
		init : function(){
			...
			page.common.$gt.on("click",function(){
				if(!page.common.flg){
					...
					$("#sp-mypage").addClass("open");
					TweenLite.set(page.common.$hc,{skewX:-15, skewY:-25 , height: $("#document-inner").innerHeight()});
...
$(window).on("load",function(){
	...
	/* mypage 予約済、開催済イベントのタブ切り替え時にローダーを消す */
	var pageURL = window.location.href,
		pageID = $("body").attr("id");
	if(pageID=="mypage"){
		$(".js__tab-trigger").on("click",function(){
			var $t = $("#"+$(this).data("target")+"-event");
			$(".js__tab-trigger").removeClass("active");
			$(this).addClass("active");

			TweenMax.to($(".js__tab-content"),0.28,{
				opacity:0,
				onStart:function(){
					$(".js__tab-container").css({"height":$t.innerHeight()});
				},
				onComplete:function(){
					$(".js__tab-content").css({"display":"none"});
					$t.css({"display":"block"});
					TweenMax.to($(".js__tab-content"),0.18,{
						opacity:1,
					});
	...
	page.init();
	if($("#document-inner").height() < page.h) $("#body").addClass("resize").css({"height":page.h - $("#footer-container").height()});
